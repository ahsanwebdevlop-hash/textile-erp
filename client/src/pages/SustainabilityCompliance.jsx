...
import { Award, Plus, Calendar, CheckCircle, Sparkles, Pencil, Trash2, X } from 'lucide-react';
...
export default function SustainabilityCompliance() {
  ...
  const openEditModal = (cert) => {
    ...
    setFormData({
      ...
      supplierOrFacility: cert.supplierOrFacility,
      issueDate: cert.issueDate?.split('T')[0] || '',
      expiryDate: cert.expiryDate?.split('T')[0] || '',
      status: cert.status || 'Active',
      scopeNotes: cert.scopeNotes || ''
    });
    setErrorMessage('');
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/compliance/${editingItem._id}`, formData);
      } else {
              ...
              <div>
                <div className="flex justify-between items-start mb-3 gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button onClick={() => openEditModal(cert)} className="p-1 text-gray-400 hover:text-indigo-600 rounded-lg hover:bg-gray-50">
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => setDeleteConfirm(cert)} className="p-1 text-gray-400 hover:text-red-600 rounded-lg hover:bg-gray-50">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl my-auto">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{editingItem ? 'Edit Certificate' : 'Register Compliance Certificate'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" className="btn-primary">{editingItem ? 'Update Certificate' : 'Register Certificate'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl">
            ...
            <h3 className="font-bold text-gray-900 text-base mb-2">Delete Certificate?</h3>
            <p className="text-xs text-gray-500 mb-6">Delete certificate {deleteConfirm.certificateNumber}?</p>
